...
class WrapperAdamLMSampler(AdamLMSampler):
    ...
    def _impl_sampling(self, net, noise, label_ids, condition, uncondition, token_lengths, extra_dict=None):
        batch_size = noise.shape[0]
        cfg_condition = condition
        x = noise
        t_cur = torch.zeros([batch_size,]).to(noise.device, noise.dtype) + local_config.ts / 10
        for i  in range(self.num_steps-1, self.num_steps):
            ...
            attention_maps = net(cfg_x, cfg_t, cfg_condition, label_ids, token_lengths, extra_dict=copy_extra_dict)
